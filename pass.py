...
def littab():
    ...
    x = [line.strip().split('\t') for line in lit.readlines()]
    print(tabulate(x,headers=["LITERALS","TABLE"] ,tablefmt="fancy_grid"))
#-----------------------------------------------------------------------------------------------------------------------------------
...
def pooltab2():
    ...
    print("\n\n")
    print(" - POOL TABLE -")
    print(tabulate([(i,) for i in pooltab], headers=["POOL TABLE"], tablefmt="fancy_grid"))
...
def symbol():
    global symtab 
    symbol_addresses.extend([entry[0] for entry in symtab.values()])
    print("\n\n")
    print(" ---  SYMBOL TABLE ---")
    symbol_data = [(symtab, data[0]) for symtab, data in symtab.items()]
    print(tabulate(symbol_data, headers=["SYMBOL", "VALUE"], tablefmt="fancy_grid"))
    print("\n\n")
...
# Count the number of times LC changes
lc_changes_count = sum(1 for i in range(1, len(lc_values)) if lc_values[i] != lc_values[i - 1])
print("\nNumber of times LC changed it's position -: ",lc_changes_count)
print("\n")
literal_count = 0
print("Total number of literals encountered: ", literal_count)
print("\n")
#-----------------------------------------------------------------------------------------------------------------------------------

#VISUALIZATION FOR NUMBER OF TIME LC CHANGED IT'S POSITION , POOT TABLE , LITERALS TABLE , SYMBOL TABLE

#-----------------------------------------------------------------------------------------------------------------------------------

# bar chart to visualize the number of times LC changes
plt.figure(figsize=(8, 6))
plt.bar(['LC Changes'], [lc_changes_count], color='b')
plt.ylabel('Count')
plt.title('Number of Times LC Changes')
plt.show()

#-----------------------------------------------------------------------------------------------------------------------------------

# Create a line plot for the literals table
plt.figure(figsize=(8, 6))
plt.plot(range(len(literal_addresses)), literal_addresses, 'bo', label='Literal Addresses')
plt.xlabel('Literal Index')
plt.ylabel('Address')
plt.title('Literals Table Addresses')
plt.legend()
plt.show()

#-----------------------------------------------------------------------------------------------------------------------------------

# Create a line plot for the pool table
plt.figure(figsize=(8, 6))
plt.plot(pooltab, [1] * len(pooltab), 'ro', label='"LTORG" in Pool Table')
plt.xlabel('Location Counter (LC)')
plt.ylabel('Pool Table Entries')
plt.title('Pool Table Entries')
plt.yticks([])
plt.legend()
plt.show()
...
# Create a line plot for the symbol table
plt.figure(figsize=(8, 6))
plt.plot(range(len(symbol_addresses)), symbol_addresses, 'go', label='Symbol Addresses')
plt.xlabel('Symbol Index')
plt.ylabel('Address')
plt.title('Symbol Table Addresses')
plt.legend()
plt.show()
